# MANTRA Layer 1: Implementation Specifications

**TYPE**: Specification Repository
**STATUS**: FROZEN (per MANTRA-L1-FREEZE-001)
**AUTHORITY**: Human Decision Only
**GOVERNING LAW**: MANTRA-LAW-001

---

## Notice

Layer 1 contains implementation specifications.

Layer 1 does NOT contain law, binding decisions, or authority grants.

Layer 1 is subordinate to Layer 0 in all cases.

---

## Boundary Constraint

All Layer 1 documents are governed by:

**MANTRA-LAYER-1-BOUNDARY-001** (Implementation Boundary Rules)

This boundary specification prevents Layer 1 from violating Layer 0.

---

## Documents

| Document | Type | Purpose | Status |
|----------|------|---------|--------|
| MANTRA-LAYER-1-BOUNDARY-001 | Boundary Specification | Defines what Layer 1 may and must not do | FROZEN |
| MANTRA-L1-IMPL-VALIDATOR-001 | Implementation Specification | Validator implementation requirements | FROZEN |
| MANTRA-L1-IMPL-DECISION-STORE-001 | Implementation Specification | Decision Store storage requirements | FROZEN |
| MANTRA-L1-IMPL-PUBLIC-READ-API-001 | Implementation Specification | Public Read API access requirements | FROZEN |
| MANTRA-L1-FREEZE-001 | Governance Declaration | Layer 1 freeze declaration | ACTIVE |
| MANTRA-L1-CORRECTION-001 | Correction Record | Post-audit documentation corrections | APPLIED |

---

## Layer 0 Supremacy

Layer 0 is constitutionally superior.

Layer 1 implements. Layer 1 does not redefine.

If conflict exists between Layer 1 and Layer 0:

**Layer 0 prevails. Layer 1 is corrected or discarded.**

---

## Document Naming Convention

| Prefix | Purpose | Baseline Status |
|--------|---------|-----------------|
| `MANTRA-LAYER-1-BOUNDARY-` | Layer governance documents | In frozen baseline |
| `MANTRA-L1-IMPL-` | Implementation specifications | In frozen baseline |
| `MANTRA-L1-FREEZE-` | Freeze declarations | In frozen baseline |
| `MANTRA-L1-CORRECTION-` | Correction records | Documentation only |
| `MANTRA-ARCH-` | Technical architecture documents | Not in frozen baseline |
| `MANTRA-PROC-` | Operational procedures | Not in frozen baseline |
| `MANTRA-GUIDE-` | Integration guides | Not in frozen baseline |

---

## Compliance Requirement

Every Layer 1 specification MUST include a compliance declaration per MANTRA-LAYER-1-BOUNDARY-001 §9.2.

---

## Prohibited Content

Layer 1 MUST NOT contain:

- Constitutional law
- Binding decisions modifying Layer 0
- Alternative schemas
- Authority grants
- AI permission expansions

See MANTRA-LAYER-1-BOUNDARY-001 §4 and §5 for complete prohibitions.

---

**END OF README**
