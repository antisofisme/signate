# ATLAS_MANTRA: Phase 1 - DB Conformance Report
...
```bash
cd /mnt/f/WINDSURF/neliti_code/signate/ATLAS_MANTRA
...
```bash
docker exec -i mantra-postgres psql -U mantra_owner -d atlas_mantra \
...
```bash
docker exec -i mantra-postgres psql -U mantra_owner -d atlas_mantra -c \
