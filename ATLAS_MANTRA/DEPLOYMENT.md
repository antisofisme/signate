...
rsync -avz --exclude '__pycache__' --exclude '.git' \
  ./ATLAS_MANTRA/ root@31.97.111.175:/root/atlas-mantra/
```

### Step 2: Build Docker Image (BEFORE nomad job run!)
```bash
ssh root@31.97.111.175 "cd /root/atlas-mantra/backend && docker build -t atlas-mantra-api:v1.0.0 ."
```
...
```bash
ssh root@31.97.111.175 "cd /root/atlas-mantra/nomad && nomad job run -namespace=mantra mantra-backend.nomad"
```
...
# Run migrations
for f in /root/atlas-mantra/backend/migrations/*.sql; do
  ssh root@31.97.111.175 "docker exec -i $PG_CONTAINER psql -U mantra_owner -d atlas_mantra < $f"
done
```
...
│    ▼         ▼                                         │
│ ┌──────┐  ┌──────────┐                                 │
│ │ PG   │  │ Backend  │◀── atlas-mantra-api:v1.0.0     │
