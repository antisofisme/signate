...
This document establishes the **security philosophy and constitutional framework** for ARSAKA_PANDAWA. Security in large cloud-based, multi-tenant applications is not a feature or checklist—it is **enforced discipline built into architecture**.
...
These five principles guide every architecture decision in ARSAKA_PANDAWA:
...
Security in ARSAKA_PANDAWA is not:
