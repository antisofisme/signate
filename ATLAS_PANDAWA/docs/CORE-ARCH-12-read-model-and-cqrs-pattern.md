...
This document describes the **Command Query Responsibility Segregation (CQRS)** pattern used in ATLAS_PANDAWA. CQRS separates the write path (commands) from the read path (queries), optimizing each independently.
