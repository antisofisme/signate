...
This document establishes the **security philosophy and constitutional framework** for ATLAS_PANDAWA. Security in large cloud-based, multi-tenant applications is not a feature or checklist—it is **enforced discipline built into architecture**.
...
These five principles guide every architecture decision in ATLAS_PANDAWA:
...
Security in ATLAS_PANDAWA is not:
