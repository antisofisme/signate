...
class ChatOrchestrator:
    ...
    async def chat(
        ...
        # Retrieve relevant context via RAG (Phase 2)
        rag_context: Optional[RetrievalResult] = None
        retrieved_doc_ids: List[str] = []
        rag_config = self._get_rag_config(ctx)

        if use_rag and self.rag_orchestrator and rag_config.enabled:
            try:
        ...
        # Build messages for LLM (with RAG context if available)
        messages = self._build_messages(ctx, history, message, rag_context)
    ...
    def _build_messages(
        ...
        # System prompt (possibly augmented with RAG instructions)
