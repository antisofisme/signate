...
export function ContentAssignmentModal({
  ...
  useEffect(() => {
    if (assignedContents.length > 0) {
      setSortedItems([...assignedContents].sort((a: any, b: any) => b.priority - a.priority));
    } else {
      setSortedItems([]);
    }
  }, [assignedData]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Update priorities based on new order (highest priority = first position)
    // Calculate new priorities: first item gets highest, decreasing for each position
    const basePriority = 100;
    const updates = newOrder.map((item, index) => ({
      contentId: item.content_id,
      newPriority: basePriority - index,
    }));

    // Update all priorities
    try {
      for (const update of updates) {
        if (update.newPriority !== sortedItems.find(i => i.content_id === update.contentId)?.priority) {
          await assignContent.mutateAsync({
            deviceId: device.id,
