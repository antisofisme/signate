...
class DecisionV3(BaseModel):
    ...
    This is the enhanced core aggregate of ARSAKA_MANTRA.
    ...
    scope: Scope = Field(..., description="Applicability scope (legacy: ORGANIZATION/DOMAIN/APPLICATION)")
    blast_radius: BlastRadius = Field(..., description="Impact level")

    # =========================================================================
    # Hierarchical Scope (DAG-based) - NEW
    # =========================================================================
    # These fields support the hierarchical scope system with umbrella decisions,
    # shared children, and inheritance. See scope_dag.py and scope_registry.py.

    scope_paths: List[str] = Field(
        default_factory=list,
        description=(
            "Hierarchical scope paths (e.g., ['fe.api', 'be.api']). "
            "Supports: umbrella (above multiple branches), shared child (under multiple parents), "
            "wildcard patterns (*.api). Empty list means entire project."
        )
    )
    scope_inheritance_type: Optional[str] = Field(
        default=None,
        description=(
            "How this decision's scope inherits: "
            "'umbrella' (above multiple branches, inherits DOWN), "
            "'shared_child' (under multiple parents), "
            "'leaf' (single path), "
            "'wildcard' (uses * pattern)"
        )
    )
    scope_applies_to: List[str] = Field(
        default_factory=list,
        description=(
            "PRE-COMPUTED: All scope paths this decision applies to (denormalized for AI retrieval). "
            "Automatically populated by scope resolver when decision is created/updated."
        )
    )
    scope_applies_to_description: Optional[str] = Field(
        default=None,
        description=(
            "Human-readable description of scope applicability for AI context. "
            "Example: 'Applies to frontend API and backend API (shared child decision)'"
        )
    )
    version: str = Field(
        ...,
    ...
    # NOTE: `tags` field REMOVED - replaced by hierarchical scope_paths
    # Old: tags: ["FE", "API", "AUTH"]
    # New: scope_paths: ["fe.api.auth"]
    ...
    @classmethod
    def convert_and_validate_constraints(cls, v):
        """
        Convert Constraint to EnhancedConstraint and validate.

        Handles backward compatibility with v2 Constraint objects.
        ...
        # Convert each item to EnhancedConstraint if needed
        converted = []
        for item in v:
            if isinstance(item, EnhancedConstraint):
                converted.append(item)
            elif isinstance(item, dict):
                # Handle dict input - fill in default values
                constraint_data = {
                    "constraint_id": item.get("constraint_id", f"C-{len(converted)+1:03d}"),
                    "statement": item.get("statement", ""),
                    "type": item.get("type") or item.get("constraint_type", "REQUIREMENT"),
                    "enforcement_level": item.get("enforcement_level", "STRICT"),
                    "automated_check": item.get("automated_check", False),
                    "check_command": item.get("check_command"),
                    "exception_process": item.get("exception_process"),
                    "exception_template": item.get("exception_template"),
                    "granted_exceptions": item.get("granted_exceptions", []),
                    "enforcement_status": item.get("enforcement_status"),
                }
                # Handle rationale field that exists in v2 Constraint but not EnhancedConstraint
                # Just ignore it for now
                converted.append(EnhancedConstraint(**constraint_data))
            elif hasattr(item, "constraint_id"):
                # It's a Constraint object (v2) - convert to EnhancedConstraint
                constraint_data = {
                    "constraint_id": item.constraint_id,
                    "statement": item.statement,
                    "type": getattr(item, "type", None) or getattr(item, "constraint_type", "REQUIREMENT"),
                    "enforcement_level": getattr(item, "enforcement_level", "STRICT"),
                    "automated_check": getattr(item, "automated_check", False),
                    "check_command": getattr(item, "check_command", None),
                    "exception_process": getattr(item, "exception_process", None),
                }
                converted.append(EnhancedConstraint(**constraint_data))
            else:
                raise ValueError(f"Invalid constraint type: {type(item)}")

        # Validate count
        if len(converted) > ContentLengthConfig.CONSTRAINTS_MAX:
            raise ValueError(
                f"Too many constraints: {len(converted)} "
                f"(max: {ContentLengthConfig.CONSTRAINTS_MAX}). "
                f"Consider grouping related constraints."
            )

        # Validate uniqueness
        ids = [c.constraint_id for c in converted]
    ...
    @model_validator(mode='after')
    def populate_scope_denormalized_fields(self):
        """
        Populate denormalized scope fields for AI retrieval.

        When scope_paths is provided, automatically compute:
        - scope_inheritance_type
        - scope_applies_to
        - scope_applies_to_description

        This enables efficient scope-aware retrieval without runtime computation.
        """
        if self.scope_paths and not self.scope_applies_to:
            try:
                # Import here to avoid circular dependency
                from .scope_registry import get_applies_to_for_retrieval

                scope_info = get_applies_to_for_retrieval(self.scope_paths)

                # Use object.__setattr__ because Pydantic models may be frozen
                object.__setattr__(self, 'scope_inheritance_type', scope_info.get("inheritance_type"))
                object.__setattr__(self, 'scope_applies_to', scope_info.get("all_scopes", []))
                object.__setattr__(self, 'scope_applies_to_description', scope_info.get("description", ""))
            except ImportError:
                # Scope registry not available (e.g., during testing)
                pass
            except Exception:
                # Don't fail validation on scope resolution errors
                pass
    ...
    def get_all_scope_tags(self) -> List[str]:
        """
        Get all scope segments as flat tags (for backward compatibility).

        Converts hierarchical scope_paths to flat tag list.
        Example: ["fe.api", "be.api"] → ["fe", "api", "be"]
        """
        tags = set()
    ...
    # =========================================================================
    # Hierarchical Scope Methods (DAG-based)
    # =========================================================================

    def refresh_scope_fields(self) -> bool:
        """
        Refresh denormalized scope fields from scope_paths.

        Call this after modifying scope_paths to update the computed fields.

        Returns:
            True if fields were updated, False if scope_paths is empty or error occurred.
        """
    ...
    def applies_to_scope(self, query_scope: str) -> bool:
        """
        Check if this decision applies to a given scope.

        Uses pre-computed scope_applies_to for efficiency.

        Args:
            query_scope: Scope path to check (e.g., "fe.api.rest")

        Returns:
            True if decision applies to the scope
        """
        # Direct match
        if query_scope in self.scope_applies_to:
            return True

        # Check if query is descendant of any applied scope
        for scope in self.scope_applies_to:
            if query_scope.startswith(f"{scope}."):
                return True

        # Empty scope_applies_to means applies to everything
        if not self.scope_applies_to and not self.scope_paths:
            return True

        return False

    def get_scope_match_score(self, query_scope: str) -> float:
        """
        Get a relevance score for how well this decision matches a scope.

        Args:
            query_scope: Scope to match against

        Returns:
            Score from 0.0 (no match) to 1.0 (exact match)
        """
        if not query_scope:
            return 1.0  # No scope filter = full match

        # Exact match in scope_paths
        if query_scope in self.scope_paths:
            return 1.0

        # Exact match in applies_to
        if query_scope in self.scope_applies_to:
            return 0.95

        # Query is descendant of a scope this applies to
        for scope in self.scope_applies_to:
            if query_scope.startswith(f"{scope}."):
                # Score decreases with distance
                depth_diff = query_scope.count(".") - scope.count(".")
                return max(0.5, 0.9 - (depth_diff * 0.1))

        # Query is ancestor of a scope this applies to
        for scope in self.scope_applies_to:
    ...
    def get_scope_description(self) -> str:
        """
        Get human-readable description of this decision's scope.

        Returns:
            Description string suitable for AI context
        """
        if self.scope_applies_to_description:
            return self.scope_applies_to_description

        if not self.scope_paths:
            return f"Applies to entire project ({self.scope.value} level)"

        if len(self.scope_paths) == 1:
            return f"Applies to: {self.scope_paths[0]}"

        return f"Applies to multiple areas: {', '.join(self.scope_paths)}"
...
class DecisionCreateV3(BaseModel):
    ...
    # Hierarchical scope (replaces old `tags` field)
    scope_paths: List[str] = Field(
        default_factory=list,
        description="Hierarchical scope paths (e.g., ['fe.api', 'be.api'])"
    )
