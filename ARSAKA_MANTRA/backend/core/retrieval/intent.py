...
┌──────────────┬────────────────┬───────────────────────────────────────┐
│ Intent       │ Strategy       │ Parameters                            │
├──────────────┼────────────────┼───────────────────────────────────────┤
│ LIST         │ BROAD          │ threshold=0.5, limit=20, expand=True  │
│ ENFORCE      │ PRECISE        │ threshold=0.8, limit=5, expand=False  │
│ UNDERSTAND   │ BALANCED       │ threshold=0.65, limit=10, expand=True │
│ REVIEW       │ EXHAUSTIVE     │ threshold=0.4, limit=50, expand=True  │
│ EXPLORE      │ BROAD          │ threshold=0.5, limit=20, expand=True  │
│ FULL         │ EXHAUSTIVE     │ threshold=0.4, limit=50, expand=True  │
│ PRD_EXPORT   │ EXHAUSTIVE     │ threshold=0.4, limit=50, expand=True  │
└──────────────┴────────────────┴───────────────────────────────────────┘
"""
...
class StrategyParams:
    """Parameters for search strategy."""
...
# Search strategy per intent
INTENT_SEARCH_STRATEGY: Dict[QueryIntent, SearchStrategy] = {
...
# Strategy parameters
STRATEGY_PARAMS: Dict[SearchStrategy, StrategyParams] = {
    SearchStrategy.BROAD: StrategyParams(
    ...
    SearchStrategy.PRECISE: StrategyParams(
    ...
    SearchStrategy.BALANCED: StrategyParams(
    ...
    SearchStrategy.EXHAUSTIVE: StrategyParams(
...
class IntentDetector:
    ...
    SIGNALS: Dict[QueryIntent, List[str]] = {
        # PRD_EXPORT - Check first (specific phrases)
        QueryIntent.PRD_EXPORT: [
            r"\bexport\s+as\b",           # "export as PRD"
            r"\bexport\s+to\b",           # "export to markdown"
            r"\bgenerate\s+doc\b",        # "generate doc"
            r"\bgenerate\s+documentation\b",
            r"\bcreate\s+prd\b",          # "create PRD"
            r"\bprd\b",                   # PRD keyword
        ...
        # REVIEW - Check before ENFORCE (has overlapping words like "code")
        QueryIntent.REVIEW: [
            r"\breview\s+(this|the|my|our)\b",  # "review this code"
            r"\bvalidate\s+(the|this|my)\b",    # "validate the API"
            r"\bcheck\s+(for|if|the|this)\b",   # "check for errors"
            r"\breview\b",
        ...
        QueryIntent.ENFORCE: [
            ...
            r"\bwrite\b", r"\bdevelop\b",
            r"\badd\b", r"\bbikin\b",
            r"\btolong\b.*\bbuat\b", r"\bbantu\b.*\b(buat|create)\b",
            # NOTE: "code" removed - too generic, conflicts with REVIEW
            # NOTE: "generate" removed - conflicts with PRD_EXPORT
        ],
        QueryIntent.UNDERSTAND: [
    ...
    def detect(self, query: str) -> IntentResult:
        ...
        strategy = INTENT_SEARCH_STRATEGY.get(winner, SearchStrategy.BALANCED)
