...
Implements the HTTP endpoints for ARSAKA_MANTRA services:
...
class MatrixCell(BaseModel):
    """Single cell in the decision matrix."""
    domain: str
    aspect: str
    decision_count: int
...
class MatrixResponse(BaseModel):
    """Response body for decision matrix - 4x4 taxonomy grid."""
    ...
    result = ListDecisionsResponse(
        decisions=[
            DecisionResponse(
                ...
                tech_stack=getattr(d, 'tech_stack', []) or [],
                # MICS Layer B Content
...
@router.get(
    ...
    summary="Get decision taxonomy matrix",
    description="""
    Returns the 4×4 Decision Taxonomy Matrix.
    ...
    **Features (Columns):**
    - A01-A04: Intent features
    - A05-A08: Architecture features
    - A09-A12: Control features
    - A13-A16: Evolution features
    """
)
async def matrix_endpoint(
    repository: DecisionRepository = Depends(get_repository)
) -> MatrixResponse:
    """Get the 4×4 decision taxonomy matrix."""
    # Check cache first
    cache = Container.get_cache()
    cache_key = "mantra:decisions:matrix"
    ...
    # Define the taxonomy grid
    domains = [d.value for d in DomainId]  # INT, ARCH, CTL, EVO
    aspects = [a.value for a in AspectId]  # A01-A16

    # Fetch all decisions
    stored_decisions = await repository.find_all_async(limit=10000, offset=0)

    # Build lookup by domain+aspect
    decision_lookup: Dict[str, Dict[str, List[Dict[str, Any]]]] = {}
    for domain in domains:
        decision_lookup[domain] = {}
        for aspect in aspects:
            decision_lookup[domain][aspect] = []

    for sd in stored_decisions:
        domain_key = sd.decision.domain_id.value
        aspect_key = sd.decision.aspect_id.value
        if domain_key in decision_lookup and aspect_key in decision_lookup[domain_key]:
            decision_lookup[domain_key][aspect_key].append({
                "decision_id": sd.decision.decision_id,
                "decision_code": sd.decision.decision_code,
                "statement": sd.decision.statement[:100] + "..." if len(sd.decision.statement) > 100 else sd.decision.statement,
                "version": sd.decision.version,
                "created_at": sd.decision.created_at.isoformat() if sd.decision.created_at else None
            })

    # Build matrix structure (4 rows × 4 columns per aspect group)
    matrix = []
    for domain in domains:
        row = []
        for aspect in aspects:
            decisions = decision_lookup[domain][aspect]
    ...
    total_decisions = sum(
        len(decision_lookup[d][a])
        for d in domains for a in aspects
    )
