...
class PostgresDecisionRepository(DecisionRepository):
    ...
    async def initialize(self) -> None:
        ...
        if self._pool is None:
            import asyncio
            try:
                # Timeout for initial connection establishment (10 seconds)
                self._pool = await asyncio.wait_for(
                    asyncpg.create_pool(
                        self.database_url,
                        min_size=1,  # Reduced min to speed up startup
                        max_size=10,
                        command_timeout=30,
                    ),
                    timeout=10.0
                ...
                raise RuntimeError("Failed to connect to database: connection timeout")
