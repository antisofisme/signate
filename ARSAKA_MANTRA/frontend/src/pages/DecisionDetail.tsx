...
import { decisionsApi, Decision, ChallengeRequest, ContentSection } from '../shared/api'
...
export default function DecisionDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Layer B: Detailed Content */}
      {(decision.detailed_content || (decision.sections?.length ?? 0) > 0) && (
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4">
            <h3 className="font-semibold text-white flex items-center gap-2">
              ...
              Detailed Specification (Layer B)
            </h3>
            <p className="text-purple-100 text-sm mt-1">
              Full documentation and technical details
            </p>
          </div>

          {/* Content Summary - Quick Overview */}
          {decision.content_summary && (
            <div className="bg-purple-50 border-b border-purple-100 p-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="text-xs font-medium text-purple-700 uppercase tracking-wide">Summary</span>
                  <p className="text-gray-700 text-sm mt-1">{decision.content_summary}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="p-6">
            {/* Structured Sections */}
            {decision.sections && decision.sections.length > 0 && (
              <div className="mb-6">
                <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-4">
                  Sections ({decision.sections.length})
                </h4>
                <div className="space-y-4">
                  {decision.sections
                    .sort((a: ContentSection, b: ContentSection) => a.order - b.order)
                    .map((section: ContentSection) => (
                      <details key={section.section_id} className="group border rounded-lg">
                        <summary className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50">
                          <div className="flex items-center gap-3">
                            <span className={`px-2 py-1 text-xs font-medium rounded ${
                              section.section_type === 'OVERVIEW' ? 'bg-blue-100 text-blue-700' :
                              section.section_type === 'RULES' ? 'bg-red-100 text-red-700' :
                              section.section_type === 'EXAMPLES' ? 'bg-green-100 text-green-700' :
                              section.section_type === 'STRUCTURE' ? 'bg-purple-100 text-purple-700' :
                              section.section_type === 'DIAGRAM' ? 'bg-yellow-100 text-yellow-700' :
            ...
            {decision.detailed_content && (
              <div>
                {(decision.sections?.length ?? 0) > 0 && (
                  <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-4">
                    Full Specification
                  </h4>
                )}
                <div className="prose prose-sm max-w-none prose-headings:text-gray-900 prose-p:text-gray-700 prose-code:bg-gray-100 prose-code:px-1 prose-code:rounded prose-pre:p-0">
                  <ReactMarkdown
                    components={{
                      code({ node, className, children, ...props }) {
                        const match = /language-(\w+)/.exec(className || '')
