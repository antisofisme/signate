...
This document specifies the **Workflow Engine** that manages approval workflows, state transitions, and notifications across PROJECT_BESAR. The Workflow Engine is a foundational service used by multiple modules (Accounting, Procurement, Approvals, etc.) to enforce business logic around approvals.
...
> **Standard**: These are the AUTHORITATIVE approval events used throughout PROJECT_BESAR (referenced in CORE-STD-20)
