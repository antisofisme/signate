...
This document describes the **Command Query Responsibility Segregation (CQRS)** pattern used in PROJECT_BESAR. CQRS separates the write path (commands) from the read path (queries), optimizing each independently.
