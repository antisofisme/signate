# ATLAS_PUGUH - PostgreSQL Database (Phase A)
...
job "puguh-postgres" {
  ...
  group "db" {
    ...
    network {
      ...
      port "db" {
        static = 5433  # Phase A: Use 5433 (avoid conflict with system PostgreSQL)
        to     = 5432  # Container internal port
      }
    }
    ...
    task "postgresql" {
      ...
      env {
        ...
        POSTGRES_PASSWORD = "CHANGE_THIS_PASSWORD_PRODUCTION"  # TODO: Use Vault in Phase B
...
EOF
      ...
      service {
        ...
        check {
          ...
          type     = "script"
          command  = "/bin/sh"
          args     = ["-c", "pg_isready -U atlas_user -d atlas_puguh"]
          interval = "30s"
          timeout  = "5s"
        }
      }
