...
```bash
# =============================================================================
# ATLAS_PUGUH — PHASE A CONFIGURATION (MINIMAL VISIBLE SYSTEM)
# =============================================================================
#
...
DATABASE_URL=postgresql+asyncpg://atlas_user:CHANGE_THIS_PASSWORD@localhost:5433/atlas_puguh
...
1. **DATABASE_URL**:
   ...
   DATABASE_URL=postgresql+asyncpg://atlas_user:K7m3Qp9Xz2Lv8Bn4Wc1Yr6Hf5Gd@localhost:5433/atlas_puguh
   ```
...
services:
  ...
  backend:
    ...
    environment:
      ...
      DATABASE_URL: postgresql+asyncpg://atlas_user:${DB_PASSWORD}@postgres:5432/atlas_puguh
    ports:
      - "8001:8001"
    depends_on:
      postgres:
...
docker exec -i atlas-postgres-phase-a psql -U atlas_user -d atlas_puguh < backend/migrations/seed_phase_a.sql
```
...
4. **Database Check**:
   ```bash
   docker exec -it atlas-postgres-phase-a psql -U atlas_user -d atlas_puguh -c "SELECT username, role FROM users;"
