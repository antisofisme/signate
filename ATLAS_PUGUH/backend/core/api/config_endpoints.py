...
async def list_functional_areas(
    ...
    query = select(FunctionalAreaModel).where(FunctionalAreaModel.tenant_id == UUID(tenant_id))
...
async def create_functional_area(
    ...
    existing = await session.execute(
        select(FunctionalAreaModel).where(
            and_(
                FunctionalAreaModel.tenant_id == UUID(tenant_id),
...
async def update_functional_area(
    ...
    result = await session.execute(
        select(FunctionalAreaModel).where(
            and_(
                FunctionalAreaModel.functional_area_id == UUID(area_id),
                FunctionalAreaModel.tenant_id == UUID(tenant_id)
            )
        )
    )
...
async def list_decision_types(
    ...
    query = select(DecisionTypeModel).where(DecisionTypeModel.tenant_id == UUID(tenant_id))
...
async def create_decision_type(
    ...
    existing = await session.execute(
        select(DecisionTypeModel).where(
            and_(
                DecisionTypeModel.tenant_id == UUID(tenant_id),
...
async def update_decision_type(
    ...
    result = await session.execute(
        select(DecisionTypeModel).where(
            and_(
                DecisionTypeModel.decision_type_id == UUID(type_id),
                DecisionTypeModel.tenant_id == UUID(tenant_id)
            )
        )
    )
...
async def list_approver_roles(
    ...
    query = select(ApproverRoleModel).where(ApproverRoleModel.tenant_id == UUID(tenant_id))
...
async def create_approver_role(
    ...
    existing = await session.execute(
        select(ApproverRoleModel).where(
            and_(
                ApproverRoleModel.tenant_id == UUID(tenant_id),
...
async def update_approver_role(
    ...
    result = await session.execute(
        select(ApproverRoleModel).where(
            and_(
                ApproverRoleModel.approver_role_id == UUID(role_id),
                ApproverRoleModel.tenant_id == UUID(tenant_id)
            )
        )
    )
