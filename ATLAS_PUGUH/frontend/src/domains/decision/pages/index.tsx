...
import { useRuleFilters, useRuleStatusBadge, useRuleForm, useActivationForm } from '../hooks'
...
export function RuleList() {
  ...
  return (
    <DomainPage
      ...
      actions={
        <Button variant="decision" onClick={() => navigate('/decision/rules/new')}>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => navigate(`/decision/rules/${rule.id}`)}
                        >
                          View
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => navigate(`/decision/rules/${rule.id}/versions`)}
                        >
...
export function RuleDetail() {
  ...
  if (isLoading) {
    return (
      <DomainPage domain="decision" title="Loading..." backTo="/decision/rules">
        <Card>
  ...
  if (error || !rule) {
    return (
      <DomainPage domain="decision" title="Error" backTo="/decision/rules">
        <Card>
  ...
  return (
    <DomainPage
      ...
      backTo="/decision/rules"
      actions={
        rule.status === 'DRAFT' && (
          <Button variant="decision" onClick={() => navigate(`/decision/rules/${id}/activate`)}>
          ...
          <Card className="mt-4">
            ...
            <CardContent className="space-y-2">
              <Button
                ...
                onClick={() => navigate(`/decision/rules/${id}/versions`)}
              >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex gap-4 pt-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => navigate(`/decision/rules/${id}`)}
              >
                Cancel
              </Button>
              <Button
                  ...
                  <TableRow key={decision.id}>
                    ...
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => navigate(`/decision/history/${decision.id}`)}
                      >
                        View
                      </Button>
                    </TableCell>
...
export function DecisionDetail() {
  ...
  if (isLoading) {
    return (
      <DomainPage domain="decision" title="Loading..." backTo="/decision/history">
        <Skeleton className="h-64 w-full" />
      </DomainPage>
    )
  }
  ...
  if (error || !decision) {
    return (
      <DomainPage domain="decision" title="Error" backTo="/decision/history">
        <Card>
          <CardContent className="py-12 text-center text-red-500">
            <AlertCircle className="h-8 w-8 mx-auto mb-2" />
