...
export const projectApi = {
  ...
  list: async (tenantId: string, limit = 100, offset = 0): Promise<ProjectListResponse> => {
    return fetchApi<ProjectListResponse>(
      `/api/v1/projects?tenant_id=${tenantId}&limit=${limit}&offset=${offset}`
    )
  },

  get: async (tenantId: string, projectId: string): Promise<Project> => {
    return fetchApi<Project>(`/api/v1/projects/${projectId}?tenant_id=${tenantId}`)
  },

  getBySlug: async (tenantId: string, slug: string): Promise<Project> => {
    return fetchApi<Project>(`/api/v1/projects/by-slug/${slug}?tenant_id=${tenantId}`)
  },
  ...
  delete: async (tenantId: string, projectId: string): Promise<SuccessResponse> => {
    return fetchApi<SuccessResponse>(`/api/v1/projects/${projectId}?tenant_id=${tenantId}`, {
      method: 'DELETE',
    })
  },
  ...
  listMembers: async (tenantId: string, projectId: string): Promise<ProjectMemberListResponse> => {
    return fetchApi<ProjectMemberListResponse>(
      `/api/v1/projects/${projectId}/members?tenant_id=${tenantId}`
    )
  },
  ...
  addMember: async (
    ...
    return fetchApi<ProjectMember>(`/api/v1/projects/${projectId}/members?tenant_id=${tenantId}`, {
      method: 'POST',
      body: JSON.stringify(data),
    })
  },
  ...
  updateMemberRole: async (
    ...
    return fetchApi<ProjectMember>(
      `/api/v1/projects/${projectId}/members/${userId}?tenant_id=${tenantId}`,
      {
        method: 'PATCH',
        body: JSON.stringify(data),
      }
