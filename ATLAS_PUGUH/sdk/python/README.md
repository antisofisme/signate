# ATLAS_PUGUH Core Service SDK (Python)

Python client library for ATLAS_PUGUH Core Service decision engine and workflow orchestration.

## Installation

```bash
pip install atlas-puguh-sdk
```
...
async def main():
    ...
    async with CoreServiceClient(base_url="http://localhost:8001") as client:
        ...
        if response.workflow_id:
            from atlas_puguh_sdk import ApproveWorkflowRequest

            approve_req = ApproveWorkflowRequest(
                tenant_id=request.tenant_id,
                approver_role="front_desk_manager"
            )

            approve_resp = await client.approve_workflow(
                response.workflow_id,
                approve_req
            )
            print(f"Workflow state: {approve_resp.current_state}")
