# ATLAS_PUGUH Quick Start Guide

Get up and running with ATLAS_PUGUH in 5 minutes.
...
```bash
# Clone repository
git clone https://github.com/atlashub/signate.git
cd signate/ATLAS_PUGUH
...
```bash
# Create database
createdb atlas_puguh

# Run migrations
cd backend
psql -d atlas_puguh -f migrations/001_initial_schema.sql
psql -d atlas_puguh -f migrations/002_rules_table.sql
# ... run all migrations 001-011

# Or run all at once
for f in migrations/*.sql; do psql -d atlas_puguh -f "$f"; done
```
...
```env
# Database
DATABASE_URL=postgresql+asyncpg://postgres:password@localhost:5432/atlas_puguh
...
- Ensure database exists: `psql -l | grep atlas_puguh`
...
*Need help? Open an issue on GitHub or contact support@atlashub.com*
