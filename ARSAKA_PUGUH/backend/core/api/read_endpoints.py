...
SECURITY: All endpoints require authentication and filter by tenant_id.
Uses RLS-enabled sessions for defense-in-depth tenant isolation.
"""
...
# SECURITY: Import authentication context
...
async def list_decisions(
    ...
    """
    List all decisions for current tenant (Phase A+ - no pagination)
    Returns: Recent decisions with basic info

    SECURITY: Requires authentication. Filters by tenant_id.
    """
    tenant_id = UUID(ctx.tenant_id)

    # SECURITY: Explicit tenant_id filter (defense-in-depth, RLS also enforces this)
...
async def get_decision_detail(
    ...
    if decision.approval_workflow_id:
        # SECURITY: Explicit tenant_id filter on workflow lookup
...
async def list_audit_events(
    ...
    SECURITY: Requires authentication. Will filter by tenant_id when implemented.

    Phase A+: EventLog model not created yet, returns empty array
    TODO Phase B: Create EventLogModel and implement actual querying with tenant_id filter
    """
    # Store tenant_id for future use when EventLogModel is implemented
    tenant_id = UUID(ctx.tenant_id)

    # Phase A+ workaround: Return empty events
    # Real implementation will query event_log table when model is created
    # SECURITY: Will include .where(EventLogModel.tenant_id == tenant_id) filter
    return {
        "events": [],
        "total": 0,
