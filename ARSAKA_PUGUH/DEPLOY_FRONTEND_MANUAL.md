...
**Target**: Deploy frontend ARSAKA_PUGUH Phase A+ ke VPS untuk accessibility dari browser.
...
```bash
# Di root project
cd /mnt/f/WINDSURF/neliti_code/signate/ARSAKA_PUGUH/frontend
...
```bash
cd ..
docker save arsaka-puguh-frontend:phase-a | gzip > frontend-image.tar.gz
...
# arsaka-puguh-frontend    phase-a   xxxxx          x minutes ago   xx MB
```
...
```bash
docker images arsaka-puguh-frontend:phase-a
```
...
```bash
ssh root@31.97.111.175 "docker images | grep arsaka-puguh-frontend"
```
...
npm run build
cd ..
docker build -t arsaka-puguh-frontend:phase-a frontend/
