# ARSAKA_PUGUH - Backend API (Phase A)
  ...
  group "api" {
    ...
    task "fastapi" {
      ...
      config {
        ...
        # image = "your-registry/arsaka-puguh-backend:phase-a"
        ...
        volumes = [
          "/root/arsaka-puguh/backend:/app:ro"  # Mount from host
        ]
      ...
      # artifact {
      #   source      = "https://github.com/your-org/arsaka-puguh/archive/main.zip"
      ...
      env {
        ...
        # JWT Configuration (secret via template below)
        JWT_ALGORITHM = "HS256"
        ...
        # API Configuration
        CORS_ORIGINS = "http://localhost:3000,http://127.0.0.1:3000,http://localhost:5173,http://puguh-frontend.service.consul,https://puguh.arsaka.io"
      ...
      # Secrets from Consul KV (Phase A) - migrate to Vault in Phase B
      template {
        data = <<EOF
DATABASE_URL=postgresql+asyncpg://arsaka_user:{{ key "puguh/db_password" }}@{{ range service "puguh-postgres" }}{{ .Address }}:{{ .Port }}{{ end }}/arsaka_puguh
JWT_SECRET_KEY={{ key "puguh/jwt_secret" }}
EOF
      ...
      service {
        ...
        tags = [
          ...
          "traefik.enable=true",
          "traefik.http.routers.puguh-backend.rule=Host(`api.puguh.arsaka.io`) || PathPrefix(`/api/v1`)",
