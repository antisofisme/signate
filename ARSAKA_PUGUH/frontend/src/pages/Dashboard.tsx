/**
 * Dashboard - Home Page
 * Overview of all 5 domains with real API data
...
import { useGetPendingWorkflows } from '@/domains/workflow/api'
import { useGetRules } from '@/domains/decision/api'
import { useGetMetricsSummary, useGetDLQEvents } from '@/domains/control/api'
import { useGetTenants } from '@/domains/tenant/api'

// Domain configurations (stats are dynamic, loaded via getDomainStats)
const domains = [
  {
    name: 'IAM',
    path: '/app/iam',
  ...
  },
  {
    ...
    features: ['Tenants', 'Members', 'Invitations', 'Isolation Check'],
    badge: 'FULL CRUD',
...
// Quick actions
const quickActions = [
  { label: 'View Pending Approvals', path: '/app/workflow/pending', icon: Clock, color: 'text-workflow' },
  { label: 'Create Rule Draft', path: '/app/decision/rules/new', icon: FileText, color: 'text-decision' },
  { label: 'View Audit Trail', path: '/app/control/audit', icon: Shield, color: 'text-control' },
  { label: 'Check Metrics', path: '/app/control/metrics', icon: TrendingUp, color: 'text-control' },
]

export function Dashboard() {
  // Fetch real data from APIs
  const { data: pendingWorkflows, isLoading: pendingLoading } = useGetPendingWorkflows()
  const { data: activeRules, isLoading: rulesLoading } = useGetRules({ status: 'ACTIVE' })
  const { data: metrics, isLoading: metricsLoading } = useGetMetricsSummary()
  const { data: dlqEvents, isLoading: dlqLoading } = useGetDLQEvents()
  const { data: tenants, isLoading: tenantsLoading } = useGetTenants()

  // Calculate stats from real data
  const pendingCount = pendingWorkflows?.length || 0
  const activeRulesCount = activeRules?.items?.length || activeRules?.meta?.total || 0
  // Use approvalRate as health indicator (if available)
  const systemHealth = metrics?.approvalRate || 99
  const dlqCount = metrics?.dlqSize || dlqEvents?.length || 0
  const tenantCount = tenants?.data?.items?.length || 0

  // Update domain stats with real data
  const getDomainStats = (name: string) => {
    switch (name) {
      case 'IAM':
        return { label: 'Active Users', value: '-' }
      case 'Tenant':
        return { label: 'Tenants', value: tenantsLoading ? '...' : String(tenantCount) }
      case 'Decision':
        return { label: 'Active Rules', value: rulesLoading ? '...' : String(activeRulesCount) }
      case 'Workflow':
        return { label: 'Pending', value: pendingLoading ? '...' : String(pendingCount) }
      case 'Control':
        return { label: 'DLQ Events', value: dlqLoading ? '...' : String(dlqCount) }
      default:
        return { label: '', value: '-' }
    }
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                ARSAKA_PUGUH - 5-Domain Decision Engine Platform
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              <div className={`p-3 ${dlqCount > 0 ? 'bg-red-100' : 'bg-green-100'} rounded-lg`}>
                <AlertTriangle className={`h-6 w-6 ${dlqCount > 0 ? 'text-red-600' : 'text-green-600'}`} />
              </div>
              <div>
                {dlqLoading ? (
                  <Skeleton className="h-8 w-12 mb-1" />
              ...
              <Card className={`h-full hover:shadow-md transition-shadow cursor-pointer border-l-4 ${domain.borderColor}`}>
                ...
                <CardContent>
                  ...
                  <div className="mb-4 p-3 bg-muted/50 rounded-lg">
                    <p className="text-2xl font-bold">{getDomainStats(domain.name).value}</p>
                    <p className="text-xs text-muted-foreground">{getDomainStats(domain.name).label}</p>
                  </div>
