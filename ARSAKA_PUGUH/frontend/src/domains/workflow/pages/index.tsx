...
import { useGetUsers } from '@/domains/iam/api'
...
export function WorkflowDetail() {
  ...
  if (isLoading) {
    return (
      <DomainPage domain="workflow" title="Loading..." backTo="/app/workflow">
        <Card>
  ...
  if (error || !workflow) {
    return (
      <DomainPage domain="workflow" title="Error" backTo="/app/workflow">
        <Card>
      ...
      actions={
        workflow.status === 'PENDING' && (
          <div className="flex gap-2">
            <Button
              variant="workflow"
              onClick={() => navigate(`/app/workflow/${id}/approve`)}
            >
            ...
            <Button
              variant="destructive"
              onClick={() => navigate(`/app/workflow/${id}/reject`)}
            >
            ...
            <Button
              variant="outline"
              onClick={() => navigate(`/app/workflow/${id}/escalate`)}
            >
            ...
            <Button
              variant="outline"
              onClick={() => navigate(`/app/workflow/${id}/delegate`)}
            >
...
// ============================================
// Workflow Delegate Page (MUTATION)
// ============================================

export function WorkflowDelegate() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { data: workflow, isLoading: loadingWorkflow } = useGetWorkflow(id || '')
  ...
  const delegateWorkflow = useDelegateWorkflow()
  ...
  const handleDelegate = async () => {
    ...
    try {
      await delegateWorkflow.mutateAsync({
        workflowId: id!,
        toUserId: selectedUserId,
        reason: reason || undefined,
      })
      navigate('/app/workflow/pending', {
        state: { message: 'Workflow delegated successfully' }
      })
    } catch (err) {
      console.error('Failed to delegate workflow:', err)
    }
    setShowConfirm(false)
  }
  ...
  return (
    ...
    >
      <MutationWarning
        action="DELEGATE this workflow"
        consequences={[
          'Transfer workflow responsibility to another user',
          'Update workflow assigned user',
          'Generate an audit trail entry',
          'Emit workflow.delegated event',
      ...
      <Card className="max-w-2xl">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="user-select">Select User to Delegate *</Label>
            <div className="space-y-2">
              <Input
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="Explain why this workflow is being delegated..."
              rows={3}
            />
          </div>
      ...
      <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to delegate this workflow to{' '}
              <strong>{selectedUser?.name}</strong> ({selectedUser?.email}).
              This will transfer responsibility for this workflow and will be recorded in the audit trail.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={delegateWorkflow.isPending}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDelegate}
              disabled={delegateWorkflow.isPending}
            >
              {delegateWorkflow.isPending ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  Delegating...
                </>
