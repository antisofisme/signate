# ARSAKA_PUGUH — Nomad Deployment Guide (Phase A)
...
Deploy ARSAKA_PUGUH to **existing Nomad stack** dengan minimal configuration:
...
nomad alloc exec -namespace=puguh $ALLOC_ID \
  psql -U atlas_user -d arsaka_puguh -f /tmp/migrations/001_create_organizations.sql
```
...
- [ ] Database accessible (`docker exec ... psql -U atlas_user -d arsaka_puguh -c "SELECT 1"`)
...
```bash
# Edit job file
nano /root/arsaka-puguh/nomad/backend-api.nomad
...
nomad alloc exec -namespace=puguh $BACKEND_ALLOC \
  psql -h puguh-postgres.service.consul -p 5433 -U atlas_user -d arsaka_puguh -c "SELECT 1"
```
...
# Database Access
docker exec -it <postgres-container-id> psql -U atlas_user -d arsaka_puguh
```
