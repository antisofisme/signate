  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Calendar Widgets</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">Create and manage calendar event displays</p>
        </div>
        <Button
          variant="primary"
      ...
      {!isLoading && widgetsData?.items && widgetsData.items.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {widgetsData.items.map((widget) => (
            <div key={widget.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow">
              {/* Widget Header */}
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-gray-800 dark:text-gray-100">{widget.name}</h4>
      ...
      {!isLoading && (!widgetsData?.items || widgetsData.items.length === 0) && (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 dark:bg-gray-900 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600">
          <CalendarIcon className="w-12 h-12 text-gray-400 dark:text-gray-500 mx-auto mb-3" />
          <p className="text-gray-600 dark:text-gray-400 font-medium">No Calendar Widgets</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Create your first calendar widget to get started</p>
          <Button
            variant="primary"
            leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => setShowCreateModal(true)}
